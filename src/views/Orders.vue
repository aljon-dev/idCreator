  ...
  <table class="min-w-full table-auto">
    <thead class="bg-gray-100">
      <tr>
        ...
        <th class="px-4 py-4 text-center text-xs font-medium text-gray-500 uppercase">Shipping</th>
        <th class="px-4 py-4 text-center text-xs font-medium text-gray-500 uppercase">Tracking</th>
      ...
      <tr v-for="order in filteredOrders" :key="order.id">
        ...
        <td class="px-4 py-4 text-center whitespace-nowrap">
          <div class="flex items-center justify-center">
            <div :class="getShippingDot(order.shippingService)" class="w-2 h-2 rounded-full mr-1"></div>
            <span :class="getShippingServiceStyle(order.shippingService)">
              {{ order.shippingService }}
            </span>
          </div>
        </td>
        <td class="px-4 py-4 text-center whitespace-nowrap">{{ order.trackingCode }}</td>
        <td class="px-4 py-4 text-center whitespace-nowrap">
...
>
  <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-6xl overflow-y-auto max-h-[95vh]">
    ...
    <div class="mt-6 text-right">
      ...
      <button class="px-4 py-2 rounded bg-[#DB551B] text-white">Save Changes</button>
    </div>
  </div>
</div>
</div>
...
const filteredOrders = computed(() => {
  return orders.value.filter(order => {
    const matchSearch = [
      ...
      order.customerName,
      order.trackingCode,
    ].some(val => val.toLowerCase().includes(searchTerm.value.toLowerCase()))
...
const uniqueStatuses = computed(() => [...new Set(orders.value.map(order => order.status))])
...
const importedEditTemplate = ref(null)

const handleEditTemplateUpload = (event) => {
  const file = event.target.files[0]
  if (file && file.type.startsWith('image/')) {
    const reader = new FileReader()
    reader.onload = () => {
      importedEditTemplate.value = reader.result
...
const startCount = computed(() =>
  filteredOrders.value.length === 0 ? 0 : (currentPage.value - 1) * pageSize + 1
)

const endCount = computed(() =>
  Math.min(currentPage.value * pageSize, filteredOrders.value.length)
)
